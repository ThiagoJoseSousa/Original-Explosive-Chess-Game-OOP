function gameController() {
  ...
  class Players {
      ...
      this.pieces.forEach((item) => {
        if (!item.dead) {
          let piece = document.querySelector(
            `[data-coords="${item.coords[0]}${item.coords[1]}"]`
          );
    ...
    attackChoice(newX, newY, board, promoting = false) {
      ...
      explodeAttack.addEventListener("click", (e) => {
        ...
        otherCanPlay(board);
        document
          .querySelector(`[data-coords="${newX}${newY}"]`)
          .classList.add("exploding");
      });
    ...
    getPossibleMoves(coords, board) {
      ...
      return possibilities.reduce((moves, possibility) => {
        ...
        if (
          ...
          if (
            this.checkIfEmpty(possibility[0], possibility[1], board) ||
  ...
  class King extends Pieces {
    ...
    getPossibleMoves(coords, board) {
      ...
      if (
        this.start &&
        this.checkIfEmpty(x + 2, y, board) &&
        this.checkIfEmpty(x + 1, y, board) &&
        board[x + 3][y] &&
        board[x + 3][y].start
      ) {
      ...
      if (
        ...
        this.checkIfEmpty(x - 1, y, board) &&
        this.checkIfEmpty(x - 2, y, board) &&
        this.checkIfEmpty(x - 3, y, board) &&
        board[x - 4][y] !== undefined &&
        board[x - 4][y].start
      ) {
        legalMoves.push([x - 2, y]);
      }
  ...
  class Pawn extends Pieces {
    ...
    getPossibleMoves(coords, board) {
      ...
      if (
        this.start &&
        this.color === "white" &&
        this.checkIfEmpty(x, y + 1, board) &&
        this.checkIfEmpty(x, y + 2, board)
      ) {
        allMoves.push([x, y + 2]);
      }
      if (
    ...
    promoteBox(board, attacking = false, player) {
      //UI creation
      const promotingSquare = document.querySelector(
        `[data-coords="${this.coords[0]}${this.coords[1]}"]`
      );
      const choiceBox = document.createElement("div");
      ...
      for (let i = 0; i < piecesOption.length; i++) {
        ...
        choiceImg.setAttribute(
          "src",
          `../../public/images/pieces/${this.color} ${piecesOption[i][1]}.png`
        );
  ...
  function play(e, board) {
    ...
    if (
      this.displaying.type === "pawn" &&
      !this.displaying.promoted &&
      (newY === 0 || newY === 7)
    ) {
